---
title: open-frame init
description: Scaffold a new workspace.
---

```npm
npx @open-frame/cli init [directory]
```
...
| `--locale <code>`         | prompted      | Frame UI language. One of `en`, `zh-TW`, `zh-CN`, `ja`.                  |
...
and writes the matching preset into `open-frame.config.ts` (see the
...
The dev server boots, opens the frame browser, and you're ready for your
agent to take over.
