...
port). Hot module reload is wired up for both the runtime and your frame
files — saving an `index.tsx` updates the canvas without losing frame state.
...
The skills check also obeys the `OPEN_FRAME_SKIP_SKILLS_CHECK=1`
