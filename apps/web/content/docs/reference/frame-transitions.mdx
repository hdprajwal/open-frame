...
is **no default** — pages snap unless you declare a `FrameTransition`. Snap-swap
...
- **Module-level** — `export const transition` from the frame module. Every
...
attached to the frame you're heading toward, not the one you're leaving.
...
Use `--of-dir` inside `calc()` when you genuinely need to mirror motion:
...
<Callout type="warn">
  ...
  [GitHub](https://github.com/hdprajwal/open-frame/issues).
</Callout>
...
reserve a second for cover frames and a third for genuine section breaks.
