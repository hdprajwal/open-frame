...
the language picker in the frame UI (next to the theme toggle) — your choice
is remembered in the browser.

The locale only affects the **runtime UI** that open-frame renders around
your frames (the home screen, frame browser, inspector, asset panel,
presenter mode, toasts, etc.). It does **not** translate frame content —
...
All presets are re-exported from the `@open-frame/core/locale` subpath:

```ts
import { en, zhTW, zhCN, ja } from '@open-frame/core/locale';
```
...
`Locale` type from `@open-frame/core`.

```ts title="open-frame.config.ts"
import type { OpenFrameConfig, Locale } from '@open-frame/core';
import { en } from '@open-frame/core/locale';
...
const config: OpenFrameConfig = { locale: fr };
export default config;
```
