...
box with a hint label. Drop it into a frame whenever the layout calls for a
...
The asset path must live under `./assets/` relative to the frame. If you skip
...
frame's topic — a product-intro deck (one screenshot per feature), an offsite
...
See the **Image placeholders** section of the `frame-authoring` skill for the
