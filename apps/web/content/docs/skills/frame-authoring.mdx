---
title: /frame-authoring
description: The technical reference the agent reads before writing a frame.
---

`/frame-authoring` is the canonical spec for everything inside
`frames/<id>/`. The agent reads it before writing — file contract, the
1920×1080 canvas, type scale, palette, layout vocabulary, the self-review
checklist, and the anti-pattern list. `/create-frame` defers to it for the
*how*.

You don't usually invoke `frame-authoring` directly. Other skills do.
