...
- `themes/<id>.md` — palette, type stack, layout vocabulary, fixed
  ...
  `/create-frame` reads on its next run.
- `themes/<id>.demo.tsx` — a runnable 2–3 page mini-frame that inlines
...
preview, a demo without its markdown can't be picked by `/create-frame`.

The input can be:

- **An existing deck.** The skill scans `frames/<id>/index.tsx`, distils the
...
Once the file exists, mention it by name on the next `/create-frame`:
