...
![Open-frame inspector with the property panel docked beside the canvas, editing a selected element's typography and colour](/assets/screenshots/inspector.webp)

The inspector is a dev-only overlay built into the open-frame runtime. Toggle
it from the toolbar (or press `i`), then click any element on the frame.

## Two surfaces

The inspector exposes two ways to edit a frame:
...
- **Image src** — swap to any file in the deck's assets folder or the global `assets/` folder; the Replace picker has a **Frame / Global** toggle and rewrites the import accordingly (`./assets/...` or `@assets/...`).
...
The inspector pins the note as an `@frame-comment` marker in the source. The
...
tag, and text snippet — to `node_modules/.open-frame/current.json` whenever
you click. Ask your agent *"make this bigger"* and the
[`/current-frame`](/docs/skills/current-frame) skill reads that file
