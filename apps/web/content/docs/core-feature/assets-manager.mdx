...
![Open-frame assets panel listing frame files with the svgl logo search open](/assets/screenshots/assets-manager.webp)

Assets come in two scopes:

- **Frame-local** — one-off files only used by a single deck. They live next to
...
└── frames/
    └── q2-launch/
        ├── index.tsx
        └── assets/          # frame-local — only this deck
            ├── cover.png
            ├── chart.svg
            └── intro.mp4
```
...
```tsx
import cover from './assets/cover.png';      // frame-local
import chart from './assets/chart.svg';      // frame-local
...
The dev server has an **Assets** panel with a **Frame / Global** scope toggle
at the top:

- **Frame** scope reads and writes `frames/<id>/assets/`.
...
scope is active (frame-local or global) and you can `import` it immediately.

## Cross-bundler URL handling

Vite (the dev server) returns asset imports as URL **strings**. Other bundlers
return an object like `{ src, width, height }`. If you want a frame to render
