...
description: Technical reference for writing or editing open-frame pages — file contract, format-driven canvas (1920×1080 default), type scale, layout, palette/visual direction, and assets. Consult this whenever you are about to write or modify any file under `frames/<id>/`, including from inside the `create-frame` or `apply-comments` workflows, or for any ad-hoc frame edit. Triggers on phrases like "edit frame", "tweak this page", "fix the layout", "change the palette", "investigate the open-frame framework", "how do frames work here".
---

# Authoring open-frame pages

This skill is the **technical reference** for everything that happens inside `frames/<id>/index.tsx`. It does not own a workflow:

- `create-frame` owns "draft a new deck" — it asks the user scoping questions, then delegates the *how* to this skill.
...
- `current-frame` resolves deictic references ("this page", "the frame I'm on") to a concrete `frameId` + `pageIndex`. Consult it **first** when the user references the current frame without naming it, then come back here for how to edit it.
- Any ad-hoc frame edit (manual tweak, one-off fix) should also consult this skill before touching the file.
...
- Put the frame under `frames/<kebab-case-id>/`.
- Entry is `frames/<id>/index.tsx`. Images/videos/fonts go under `frames/<id>/assets/`.
...
- A frame is **one `index.tsx` plus `assets/`** — nothing else. Do not create sibling `.tsx`/`.ts` files (`Card.tsx`, `components/`, `helpers.ts`, etc.); helper components and constants go inside `index.tsx`. Do not create `README.md` or other prose files either.
...
export const meta: FrameMeta = {
  ...
  // format: 'carousel', // omit for the default 1920×1080 frame canvas
};
...
- `meta.title` (optional) shows in the frame header. Default is the folder name.
- The frame id is the kebab-case folder name. Pick something short and descriptive (`q2-roadmap`, `team-offsite-2026`).
- `meta.theme` (optional) marks the frame as built from a theme under `themes/`. The id must match a `<id>.md` basename. Surfaces a back-link chip on the frame card and lists the frame on `/themes/<id>`. Omit if the frame isn't derived from a registered theme.
...
- `meta.createdAt` is an **ISO 8601 string literal** (e.g. `'2026-05-16T12:00:00Z'`) set once when the frame is scaffolded. The home page uses it for the default "newest first" sort. Always include it on new frames — **immediately before writing the file, run `node -e "console.log(new Date().toISOString())"` via Bash and paste the exact output** as the value. Don't type a timestamp from memory — you will get the date or time wrong. Must be a plain string literal (no `new Date(...)` or imports in the frame itself) — the framework reads it via a regex at build time, not by evaluating the module.

## Editing an existing frame

A finished frame commonly runs 1000–1800 lines. When you only need to touch one page, **don't read the whole file** — locate the page first, then read just that range:

```bash
grep -n ": Page = " frames/<id>/index.tsx
```
...
The canvas does **not** scroll. Anything below the canvas height is silently cropped. Before writing JSX, do the math on paper and confirm the page fits. This is the #1 cause of broken frames — assume you will overflow unless you've checked.
...
Read on a phone, thumb-scrolling a feed — far smaller than a projected frame, so type runs **bigger relative to the canvas**, not smaller:
...
- **No page counters, no footers, no "01 / 12".** `useFramePageNumber` and swipe cues are meaningless on a single frame.
...
- **Load the stylesheet once, in `<head>` — never inside a per-page component.** Every page is mounted live at the same time (thumbnail rail, overview grid, and the PDF print root), so a `<style>@import>` / `<link>` rendered inside a `Page` registers the whole `@font-face` set once *per page*. Inject it once, idempotently:
  ...
  if (typeof document !== 'undefined' && !document.getElementById('of-webfont')) {
...
If `themes/<id>.md` exists at the project root and the frame is meant to follow it, **the theme file overrides the defaults in this skill** — its palette, typography, layout padding, and Title/Footer components are authoritative. Read the theme file before applying anything else in this section.

Themes are produced by the `create-theme` skill and are pure documentation: copy the palette and the paste-ready Title / Footer / Eyebrow components straight into your frame. If the theme's frontmatter has `mode: dark` or `mode: light`, treat that as the frame's background mode (e.g. when picking which logo variant to import).

## Design system (opt-in, per-frame)

A frame can declare its own typed design tokens at the top of `index.tsx`:

```tsx
import type { DesignSystem, Page } from '@open-frame/core';
...
The shape is intentionally minimal — it only covers what the Design panel can currently tweak. Anything outside this set (heading sizes, spacing, motion, extra palette colors) belongs as plain hard-coded constants in the frame file.

There are **two consumption surfaces**, and you should mix them inside the same frame:

- **`var(--of-X)` for visual properties (color, font, font-size, radius)** — these get instant updates while the user drags a slider in the Design panel, before any file write.
  ```tsx
  <div style={{ background: 'var(--of-bg)', color: 'var(--of-text)', borderRadius: 'var(--of-radius)', fontFamily: 'var(--of-font-body)', fontSize: 'var(--of-size-body)' }}>
  ```
  Available vars: `--of-bg`, `--of-text`, `--of-accent`, `--of-font-display`, `--of-font-body`, `--of-size-hero`, `--of-size-body`, `--of-radius`.
...
The dev UI has a **Design** button in the frame header (next to Inspect). Edits update an in-memory draft and the live-preview overlay; a floating Save / Discard bar at the bottom of the canvas commits or reverts. The const stays the single source of truth — production builds bake the saved values.

**Default to using it.** Every new frame should declare a `design` const so it stays tweakable from the panel after generation — this is the expected baseline. Only fall back to the local `palette` constants pattern (see starter template) for a one-off frame whose palette is intentionally locked and not meant to be re-themed. Both styles can coexist across frames — the panel only operates on the *currently viewed* frame.
...
- `DesignSystem` must be imported from `@open-frame/core` (the panel adds the import automatically when creating a fresh block).
...
const Cover: Page = () => (
  ...
  >
    <div style={{ fontSize: 28, color: 'var(--of-accent)', letterSpacing: '0.2em' }}>
      CHAPTER 01
    </div>
    <h1
      style={{
    ...
    <p style={{ fontSize: 'var(--of-size-body)', color: muted, maxWidth: 1200 }}>
      A short subtitle that explains what this frame is about.
    </p>
  </div>
);

const Content: Page = () => (
  <div style={{ ...fill, background: 'var(--of-bg)', color: 'var(--of-text)', padding: 120 }}>
    <h2 style={{ fontFamily: 'var(--of-font-display)', fontSize: 80, fontWeight: 800, margin: 0 }}>
      Section heading
    </h2>
    <ul style={{ fontSize: 'var(--of-size-body)', lineHeight: 1.6, marginTop: 64, paddingLeft: 48 }}>
...
**Frame-local assets** live under `frames/<id>/assets/` — anything one-off to a single frame. Import them as ES modules:
...
A `themes/*.md` file may name an asset path in its prose (e.g. "use `@assets/logos/acme.svg` in the title slot"); the frame imports it explicitly.

Skip the `assets/` folder entirely for pure-text frames.
...
```tsx
import { ImagePlaceholder } from '@open-frame/core';
...
**Do not use a placeholder** for decoration, generic "stock photo" filler, hero imagery on a text-heavy frame, or anywhere a typographic / iconographic / illustrative solution would do. If you can carry the page with type, layout, and color — do that. Empty placeholders the user has to fill are friction; only spend that friction when the alternative is worse.
...
If a footer shows the current page (`03 / 12`, `Page 3`, etc.), read it from `useFramePageNumber()` — **never hardcode** `n` / `TOTAL`. Inserting, reordering, or deleting a page would otherwise force you to retouch every footer.
...
`current` is 1-indexed (matches what readers see) and `total` is the frame's page count. The hook works in every render context (main viewer, thumbnails, overview grid, present mode, presenter window, HTML/PDF export) — the same `<Footer />` JSX is correct everywhere. Call the hook inside a component that's used **per page**; don't try to call it at module top level.
...
`frames/build-on-reveal/` is the canonical worked example; study it before authoring a stepped page.

```tsx
import { Step, Steps } from '@open-frame/core';
...
The framework can run an enter/exit animation between every frame change. There's **no default** — pages snap unless you declare a `FrameTransition`. Snap-swap is a perfectly tasteful default; only opt in when motion adds something.
...
The framework also exposes `--of-dir` (`1` forward, `-1` backward) and `data-of-dir` on the wrapper, so a single keyframe can mirror direction without a JS callback.
...
Use this set as a starting point. Pick one as the deck's house transition; optionally reserve a second for hero/cover frames and a third for genuine section breaks. The CSS-`calc` + `--of-dir` trick lets a single definition mirror itself on backward navigation when needed.
...
const dissolve: FrameTransition = {
  duration: 240,
...
Most tasteful tools don't mirror on backward navigation. When you genuinely need to — e.g. a horizontal slide that should reverse — use `--of-dir` inside `calc()`:
...
- ❌ `translateX(100%)` slide-from-side — iOS modal / PowerPoint Push; not a frame change.
...
- ❌ Duration > 350 ms for a standard frame change — drags.
...
Define the component **in the same `index.tsx`**, alongside the `Page` components. Never split it into a sibling file like `Card.tsx` — a frame is always a single `index.tsx` plus its `assets/`.
...
- Home page lists every folder under `frames/`.
- Clicking a frame shows a left thumbnail rail, main page, prev/next, page counter.
...
- [ ] `frames/<id>/index.tsx` `export default`s a non-empty `Page[]`.
...
- [ ] Frame declares a top-level `export const design: DesignSystem = { … }` and references the values via `var(--of-X)` (use `design.X` only when you need a JS number for arithmetic). Only omit the `design` const for a one-off frame whose palette is intentionally locked.
- [ ] One idea per page.
...
- [ ] All imported assets exist on disk — frame-local under `frames/<id>/assets/`, or global under `assets/` (imported via `@assets/...`).
...
- [ ] If a `FrameTransition` is declared, every page sits in one family — same duration band (140–280 ms), same easing pair, same out-then-in stagger, magnitude under 12 px / 3%. No six-different-vocabularies decks. When in doubt, omit transitions entirely.
- [ ] Nothing outside `frames/<id>/` was edited.
...
- ❌ Creating `README.md` or other prose files inside the frame folder.
- ❌ Editing `package.json`, `open-frame.config.ts`, or other frames.
