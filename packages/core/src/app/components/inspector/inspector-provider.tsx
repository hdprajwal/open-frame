...
import { type FrameComment, useComments } from '@/lib/inspector/use-comments';
...
type Bucket = {
  ...
  // `data-frame-loc`, but each call site's prop literal is independent.
...
const INSTANCE_ID_ATTR = 'data-frame-instance-id';
...
export function InspectorProvider({
  ...
  const { comments, error, refetch, add, remove } = useComments(frameId);
  const { applyEdit, applyEdits } = useEditor(frameId);
  ...
  const findAnchor = useCallback((line: number, column: number, instanceId?: string) => {
    ...
    return root.querySelector<HTMLElement>(`[data-frame-loc="${line}:${column}"]`);
  }, []);
    ...
    (line: number, column: number, anchor: HTMLElement | null, ops: EditOp[]) => {
      ...
      for (const op of ops) {
        ...
        if (op.kind === 'set-style') {
          ...
          // `data-frame-loc` but distinct call-site literals; without an
  ...
  const cancelEdits = useCallback(() => {
    ...
    const root = document.querySelector<HTMLElement>('[data-inspector-root]');
    for (const b of pendingRef.current.values()) {
      const sharedEl = root?.querySelector<HTMLElement>(`[data-frame-loc="${b.line}:${b.column}"]`);
      if (sharedEl) {
  ...
  // Re-apply buffered ops onto any `[data-frame-loc]` element that gets
  // (re)mounted in the frame canvas. Without this, navigating to a
  ...
  useEffect(() => {
    ...
    const applyBuffered = (el: HTMLElement) => {
      ...
      // (stamped with its `data-frame-instance-id`) gets the buffered
    ...
    const replayAll = () => {
      ...
      observer?.disconnect();
      root.querySelectorAll<HTMLElement>('[data-frame-loc]').forEach(applyBuffered);
  ...
  useEffect(() => {
    ...
    const revalidate = () => {
      ...
      const next = root.querySelector<HTMLElement>(
        `[data-frame-loc="${selected.line}:${selected.column}"]`,
      );
  ...
  const openReplace = useCallback((anchor: HTMLElement) => {
    const loc = anchor.dataset.frameLoc;
    if (!loc) return;
