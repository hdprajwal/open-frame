...
const SHARED_ELEMENT_SELECTOR = '[data-of-shared-element]';
...
function resolveSharedElementTransition(
  sharedElements: FrameTransition['sharedElements'],
  fallbackDuration: number,
  fallbackEasing: string,
...
function collectSharedElements(root: HTMLElement): Map<string, HTMLElement> {
  ...
  for (const el of root.querySelectorAll<HTMLElement>(SHARED_ELEMENT_SELECTOR)) {
    const id = el.dataset.ofSharedElement;
...
function cloneSharedElement(source: HTMLElement): HTMLElement {
  ...
  clone.removeAttribute('data-of-shared-element');
  return clone;
}
...
function runSharedElementTransition(
  ...
  overlay.setAttribute('data-of-shared-layer', '');
...
export function FrameTransitionLayer({
  pages,
  index,
  total,
  moduleTransition,
  ...
  useLayoutEffect(() => {
    ...
    wrapper.dataset.ofDir = direction;
    wrapper.style.setProperty('--of-dir', direction === 'forward' ? '1' : '-1');
    ...
    >
      ...
      {CurrentPage ? (
        <div ref={incomingLayerRef} className="absolute inset-0">
          <FramePageProvider index={current} total={total}>
            <StepHost
              isActivePage
              entryDirection={entryDirection}
              controllerRef={activeControllerRef}
