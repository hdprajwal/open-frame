...
// PUT    /__folders/assign     assign frame to folder { frameId, folderId | null }
...
type AssignFolderBody = { frameId?: unknown; folderId?: unknown };
  ...
  server.middlewares.use('/__folders', async (req, res, next) => {
    ...
    try {
      ...
      if (method === 'PUT' && url.pathname === '/assign') {
        ...
        if (typeof body.frameId !== 'string' || !FRAME_ID_RE.test(body.frameId)) {
          return json(res, 400, { error: 'invalid frameId' });
        }
        ...
        if (folderId === null) {
          ...
          manifest.assignments[frameId] = folderId;
        }
        await writeManifest(ctx.manifestPath, manifest);
      ...
      if (idMatch) {
        ...
        if (method === 'DELETE') {
          ...
          for (const [frameId, folderId] of Object.entries(manifest.assignments)) {
            if (folderId === id) delete manifest.assignments[frameId];
          }
          await writeManifest(ctx.manifestPath, manifest);
          return json(res, 200, { ok: true });
        }
