...
export const FRAME_ID_RE = /^[a-z0-9_-]+$/i;
...
export function validateFrameName(v: unknown): string | null {
...
export async function rmFrameDir(framesRoot: string, frameId: string): Promise<boolean> {
  if (!FRAME_ID_RE.test(frameId)) return false;
  const dir = path.resolve(framesRoot, frameId);
  if (!dir.startsWith(framesRoot + path.sep)) return false;
  try {
...
export async function duplicateFrameDir(
  framesRoot: string,
  frameId: string,
  desiredId?: string,
): Promise<{ ok: true; frameId: string } | { ok: false; status: number; error: string }> {
  if (!FRAME_ID_RE.test(frameId)) return { ok: false, status: 400, error: 'invalid frameId' };

  const root = path.resolve(framesRoot);
  const srcDir = path.resolve(root, frameId);
  if (!srcDir.startsWith(root + path.sep)) {
    return { ok: false, status: 400, error: 'invalid frameId' };
  }

  try {
    await fs.access(path.join(srcDir, 'index.tsx'));
  } catch {
    return { ok: false, status: 404, error: 'frame not found' };
  }

  let newId: string;
  if (desiredId !== undefined) {
    if (!FRAME_ID_RE.test(desiredId)) return { ok: false, status: 400, error: 'invalid newId' };
    newId = desiredId;
    ...
    try {
      ...
      return { ok: false, status: 409, error: 'frame already exists' };
    } catch {}
  } else {
    let suffix = 1;
    while (true) {
      newId = suffix === 1 ? `${frameId}-copy` : `${frameId}-copy-${suffix}`;
      try {
  ...
  try {
    ...
    if (metaTitle.kind === 'unsupported') {
      return { ok: false, status: 422, error: 'could not update copied frame title' };
    }
    const title = metaTitle.kind === 'found' ? metaTitle.title : frameId;
...
export function resolveFrameEntry(framesRoot: string, frameId: string): string | null {
  if (!FRAME_ID_RE.test(frameId)) return null;
  const dir = path.resolve(framesRoot, frameId);
  if (!dir.startsWith(framesRoot + path.sep)) return null;
  // The FrameMeta contract says every frame has frames/<id>/index.tsx; we only
...
/**
 * Rewrite (or insert) the `title` field in the frame module's `export const meta`.
 *
 * Strategy:
...
export function updateMetaTitleInSource(source: string, title: string): string | null {
  ...
  const insertion = `export const meta: FrameMeta = { title: ${newLiteral} };\n\n`;
