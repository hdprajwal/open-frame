...
async function withFramesRoot<T>(fn: (root: string) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'open-frame-test-'));
  try {
...
async function writeFrame(root: string, id: string, title = id): Promise<void> {
...
describe('duplicateFrameDir', () => {
  it('duplicates a frame directory with an automatic copy id', async () => {
    await withFramesRoot(async (root) => {
      await writeFrame(root, 'cover', 'Cover');

      const result = await duplicateFrameDir(root, 'cover');

      expect(result).toEqual({ ok: true, frameId: 'cover-copy' });
  ...
  it('records the copied entry on the self-write tracker', async () => {
    await withFramesRoot(async (root) => {
      ...
      await duplicateFrameDir(root, 'cover');
  ...
  it('increments the automatic copy id when a copy already exists', async () => {
    await withFramesRoot(async (root) => {
      await writeFrame(root, 'cover');

      expect(await duplicateFrameDir(root, 'cover')).toEqual({ ok: true, frameId: 'cover-copy' });
      expect(await duplicateFrameDir(root, 'cover')).toEqual({
        ok: true,
        frameId: 'cover-copy-2',
      });
    });
  });

  it('rejects source frame ids with bad characters', async () => {
    await withFramesRoot(async (root) => {
      expect(await duplicateFrameDir(root, 'bad id')).toMatchObject({ ok: false, status: 400 });
    });
  });
  ...
  it('rejects an existing desired id', async () => {
    await withFramesRoot(async (root) => {
      await writeFrame(root, 'cover');
      await writeFrame(root, 'target');

      expect(await duplicateFrameDir(root, 'cover', 'target')).toMatchObject({
        ok: false,
        status: 409,
      });
    });
  ...
  it('rejects path traversal in the source frame id', async () => {
    await withFramesRoot(async (root) => {
      expect(await duplicateFrameDir(root, '..')).toMatchObject({ ok: false, status: 400 });
    });
  });

  it('returns not found when the source frame does not exist', async () => {
    await withFramesRoot(async (root) => {
      expect(await duplicateFrameDir(root, 'missing')).toMatchObject({ ok: false, status: 404 });
    });
  });
});

describe('validateFrameName', () => {
  it('accepts longer frame names than folder names', () => {
    expect(validateFrameName('x'.repeat(80))).toBe('x'.repeat(80));
...
describe('updateMetaTitleInSource', () => {
  it('replaces an existing single-quoted title literal', () => {
    const source = `export const meta: FrameMeta = { title: 'old' };\nexport default [];\n`;
  ...
  it('injects a fresh meta export when none exists', () => {
    ...
    expect(out).toContain("export const meta: FrameMeta = { title: 'fresh' };");
...
describe('reorderDefaultExportPagesInSource', () => {
  const withSatisfies = `import type { Page } from '@open-frame/core';
const A = () => null;
const B = () => null;
const C = () => null;
...
`;
  ...
  it('reorders a 3-element multi-line array', () => {
    ...
    expect(out).toContain("import type { Page } from '@open-frame/core';");
...
describe('removePageFromDefaultExportInSource', () => {
  const multiline = `import type { Page } from '@open-frame/core';
const A = () => null;
const B = () => null;
const C = () => null;
export default [
