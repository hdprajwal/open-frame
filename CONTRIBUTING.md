...
Thanks for your interest in improving open-studio! This guide covers the workflow for contributing to the framework itself — the `@open-studio/core` runtime, the `@open-studio/cli` scaffolder, and the supporting apps.
...
- **Ask a question or share what you're building** in [GitHub Discussions](https://github.com/hdprajwal/open-studio/discussions).
...
| Path | Package | Role |
| --- | --- | --- |
| [`packages/core`](packages/core) | `@open-studio/core` | Runtime (viewer, present mode, inspector), Vite plugin, `open-studio` dev/build CLI. |
| [`packages/cli`](packages/cli) | `@open-studio/cli` | `npx @open-studio/cli init` scaffolder + project template. |
| [`apps/demo`](apps/demo) | private | Local consumer of `@open-studio/core` via `workspace:*`. The dogfood target for the framework. |
...
```bash
git clone https://github.com/hdprajwal/open-studio.git
cd open-studio
pnpm install
```

Then run the demo against the local `@open-studio/core`:
...
Releases are cut by the maintainer via `pnpm release`, which builds `@open-studio/core` + `@open-studio/cli` and runs `changeset publish`. Contributors don't need to publish anything — just land the changeset alongside your code.

## Questions

Open a [discussion](https://github.com/hdprajwal/open-studio/discussions) — happy to help.
